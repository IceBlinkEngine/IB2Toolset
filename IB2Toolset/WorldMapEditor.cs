...
namespace IB2Toolset
{
    ...
    public partial class WorldMapEditor : DockContent
    {
                    ...
                    else if (filter == "OnMap")
                    {
                        foreach (string f in Directory.GetFiles(prntForm._mainDirectory + "\\modules\\" + prntForm.mod.moduleName + "\\tiles\\", "*.png"))
                        {
                            ...
                            foreach (Tile tile in area.Tiles)
                            {
                                ...
                                if (tile.Layer4Filename == Path.GetFileNameWithoutExtension(f))
                                {
                                    isOnMap = true;
                                    continue;
                                }
                                if (tile.Layer5Filename == Path.GetFileNameWithoutExtension(f))
                                {
                                    isOnMap = true;
                                    continue;
                                }
        ...
        private void createTileImageButtonsOLD(string filter)
        {
            try
            {
